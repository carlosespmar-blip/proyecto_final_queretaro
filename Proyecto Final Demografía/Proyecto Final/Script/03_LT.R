...
ggplot(lt_input, aes(x = age, y = log(mx), color = sex, group = sex)) +
  ...
  labs(
    title = "Tasa de mortalidad de Michoacán por año y sexo",
    x = "Edad",
    y = "log(mx)",
    color = "Sexo"
  ) +
...
ggplot(lt_input, aes(x = age, y = log(mx), color = factor(year), group = year)) +
  ...
  labs(
    title = "Evolución de la tasa de mortalidad de Michoacán",
    #    subtitle = "México",
    x = "Edad",
    y = "log(mx)"
  ) +
...
lt_output %>%
  ...
  scale_color_manual(
    values = c(
      ...
      "2021" = "orange" ,
      "Other Years"      = "black"    # Light gray for other years
    ),
    name = "Años"
  ) +
  facet_wrap( ~ sex, ) +
...
write.csv(lt_output, "data/lt_mex", row.names = F)
# -------- FIN ----------*
